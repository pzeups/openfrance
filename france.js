...
var colors = generic_color = colorbrewer.RdYlBu[3].reverse().map(function(rgb) {
	return d3.hsl(rgb);
});
...
var layer = map.append("g").attr("id", "layer"),
	spacials = layer.append("g").attr("id", "spacials").attr('transform', 'translate(' + (width * .4) + ',0)').selectAll("path"),
	features;
...
var svg = d3.select("#bar").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
...
function init() {
	spacials = spacials.data(features).enter().append("path")
    .attr("class", "spacial").attr("id", function(d, i) {
		return d.id;
...
function reset() {
	...
	spacials.select("title").text(function(d, i) {
		return d.id;
	});
...
function update() {
	...
	if (scaled) {
        ...
        // update the data
        spacials.data(features).select("title").text(function(d, i) {
	...
	setTimeout(function() {
		spacials.on('mouseover', function(d) {
			...
			d3.select('#spacialcontent')
                .html((d.properties.num > 0 ? '[' + d.properties.num + '] ' : '') + d.id + ' <span class="value">(' + d.properties[key] + ')</span> &nbsp;'
...
function initBar(data) {
	...
	d3.select("input").on("change", function() {
		sortX(svg.transition().duration(duration));
	});
...
function sortX(transition) {
    ...
    //if(data.length > 50 ) svg.select(".x.axis").selectAll("text").style('opacity', 1).filter(function(d,i) { return i%2==0 }).style('opacity', 0)
}
